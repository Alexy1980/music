...
if(isset($_POST['playlistId']) && isset($_POST['songId'])) {
    ...
    $stmt = $pdo->prepare("SELECT MAX(playlistOrder) + 1 as playlistOrder, Count(*) as playlistOrderCount FROM `playlistSongs` WHERE `playlistId` = :playlistId");
    $stmt->bindParam(':playlistId', $playlistId, PDO::PARAM_INT);
    $stmt->execute();
    $orderIdQuery = $stmt->fetch(PDO::FETCH_OBJ);
    if($orderIdQuery->playlistOrderCount == 0){
        $playlistOrder = 1;
    } else {
